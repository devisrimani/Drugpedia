...
data = pd.read_csv(r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\drugsComTrain_raw.csv")
...
#     # Get the first 5 unique conditions and reviews
...
#     # Debugging: Print filtered reviews to check what is being returned
#     # print("Filtered Reviews DataFrame:")
...
#     # Ensure there are no NaN values in distribution
#     distribution = {k: int(v) if not pd.isna(v) else 0 for k, v in distribution.items()}
    
#     # Debugging: Print distribution to check values
#     # print("Sentiment Distribution:")
#     # print(distribution)
    
#     # Create a pie chart
#     fig, ax = plt.subplots()
#     if any(distribution.values()):  # Ensure there's data to plot
...
#         # Load data directly in the view function
#         df = pd.read_csv(r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\drugsComTrain_raw.csv")
#         df = df.dropna(subset=['drugName', 'rating', 'review'])  # Ensure no NaN values in critical columns
#         df['rating'] = pd.to_numeric(df['rating'], errors='coerce')  # Convert ratings to numeric, coerce errors
#         df = df.dropna(subset=['rating'])  # Drop rows where rating could not be converted
...
def highlight_words(text, keywords):
    for word in keywords:
        # Use a yellow background for the highlighted text
...
# Load the data
data_path = r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\drugsComTrain_raw.csv"
...
def feedback_form(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # You can save the data to the database or perform other processing here
            # For example:
...
# Load datasets
df_kerala = pd.read_csv(r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\flood.csv", encoding='latin1')
...
df_up = pd.read_csv(r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\earthquakedrugs.csv", encoding='latin1')
...
medicine_dataset_path = r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\medicine_dataset.csv"  # Update this path as needed
...
# Paths to images
background_image_first_page_path = r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\image1.jpg"  # Background image 1
background_image_other_pages_path = r"D:\Desktop\STUDY\PROJECTS\CTS HACKATHON\archive drug\image2.jpg"  # Background image 2
...
def predict_sentiment(request):
    ...
    if request.method == "GET":
        ...
        if review:
            ...
            prediction = sentiment_model.predict(review_pad)
            sentiment_label = np.argmax(prediction)
            sentiment = label_encoder.inverse_transform([sentiment_label])[0]
            print(f"Predicted Sentiment: {sentiment}")  # Debug print

    return render(request, 'predict_sentiment.html', {'sentiment': sentiment})
